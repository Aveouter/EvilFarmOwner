...
Before wages or NPC state change, each otherwise valid arrival route translates the selected NPC's real collision bounds to the candidate tile and probes the first two-pixel cardinal movement used after `Game1.warpCharacter` and `NPC.Halt`. A candidate whose standing bounds or first movement bounds collide is rejected during planning. A closed gate remains a valid first step because the non-destructive vanilla controller opens it; other failures remain closed. This probe complements rather than replaces the live progress watchdog.
...
- Distinct targets or chests can still share the same dynamically blocked departure tile. A host-owned route ledger permits at most three live interaction-route failures for one crop from the same origin. Exhausting that inner budget excludes and reports only that crop, then replans the remaining target set. A second target-isolation ledger permits at most three separately stalled crops at one unchanged origin before the worker returns with the true remainder reported unreachable. Reaching any target resets both ledgers, while a changed origin starts a fresh count. Chest delivery keeps its separate three-route bound and preserves already harvested cargo through emergency storage before the worker returns.
...
Evil Farm Owner keeps the useful execution behavior but supplies its own live collision route. The current tile is removed from every precomputed controller path because a freshly warped NPC is already standing there; asking the vanilla controller to recenter inside that tile can collide with an adjacent building before the first real step. Every dynamically selected crop and chest path repeats the translated NPC-bounds first-step probe before a controller starts. A pixel-progress watchdog replans after 180 unpaused update ticks. Three failed interaction routes isolate one crop; three isolated crops at one unchanged origin return safely; three failed delivery routes stop losslessly; and return travel gets three bounded replans before safe restoration.
...
Harvest delivery first gives the complete current stack directly to the original requesting player when that player is online, on the same main farm, and their inventory can accept the whole stack. Otherwise it uses the same route map and a four-tier classification inferred directly from each chest's contents: an exact compatible stack, the same qualified item, the same exact game item category, then an empty chest. A non-empty chest with no matching category is ineligible. Within a tier, the chest with more exact/same-item evidence wins, followed by the highest matching-category purity and matching-slot count. Stable chest coordinates break the remaining tie before walking distance, so the worker's current position cannot make equally classified items jump between chests; walking distance only selects the best interaction edge of the chosen chest. Empty chests are the sole unclassified fallback and rank by greatest acceptable capacity, then stable tile. Every normal destination must accept the complete current stack. If the requester or a chest stops accepting a preflighted stack, or no classified or empty chest is reachable, the contract stops instead of deliberately splitting cargo across normal destinations. At least one ordinary player-owned main-farm chest is required before dispatch. Cargo captured immediately before a storage stop is preserved through persistent emergency overflow, explicit visible drop, quarantine, or a validated recovery record, but those emergency destinations never let the worker continue harvesting. Cargo is delivered after each harvested target, every transfer logs its item, quality, count, destination, and remainder, and settlement audits harvested count against requester inventory, chest, emergency storage, visible drop, and unresolved totals.
