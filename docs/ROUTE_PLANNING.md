...
- Distinct targets or chests can still share the same dynamically blocked departure tile. A second host-owned ledger therefore permits at most three consecutive failures for the same travel purpose and origin. Success or movement to another origin resets that purpose's budget. Exhausted crop travel returns safely with the remainder reported unreachable; exhausted chest travel stops the contract and preserves already harvested cargo through emergency storage before the worker returns.
...
Evil Farm Owner keeps the useful execution behavior but supplies its own live collision route. The current tile is removed from every precomputed controller path because a freshly warped NPC is already standing there; asking the vanilla controller to recenter inside that tile can collide with an adjacent building before the first real step. Every dynamically selected crop and chest path repeats the translated NPC-bounds first-step probe before a controller starts. A pixel-progress watchdog replans after 180 unpaused update ticks, at most three target or delivery paths may fail consecutively from one origin, and return travel gets three bounded replans before safe restoration.
